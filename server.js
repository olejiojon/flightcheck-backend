...
app.get('/', (req, res) => {
  res.json({ status: 'ok', app: 'FLIGHT CHECK API', key_set: !!API_KEY });
});
...
app.get('/flight', async (req, res) => {
  ...
  try {
    const url = `http://api.aviationstack.com/v1/flights?access_key=${API_KEY}&flight_iata=${encodeURIComponent(iata)}&limit=1`;
...
app.get('/registration', async (req, res) => {
  ...
  try {
    const url = `http://api.aviationstack.com/v1/flights?access_key=${API_KEY}&aircraft_registration=${encodeURIComponent(reg)}&limit=1`;
    const r = await fetch(url);
    const data = await r.json();
    if (data.error) return res.status(400).json(data);
    res.json(data);
